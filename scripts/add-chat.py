#!/usr/bin/env python3
"""Wire up person-to-person chat on the deployed backend.
...
MIGRATION_ID = "c7d8e9f0a1b2"
MIGRATION_NAME = "messaging"
...
MIGRATION_TEMPLATE = '''"""Person-to-person messaging: conversations, participants, messages.
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("kind", sa.Text(), nullable=False, server_default=sa.text("'direct'")),
        sa.Column("title", sa.Text(), nullable=True),
        # Unique so a direct thread cannot be created twice and split its own history.
        # NULL for groups, and NULLs do not collide in a unique index.
        sa.Column("direct_key", sa.Text(), nullable=True, unique=True),
        sa.Column(
            "created_by",
            postgresql.UUID(as_uuid=True),
    ...
    # Every conversation-list query starts "threads for this user".
    op.create_index(
        "ix_participants_user", "conversation_participants", ["user_id"]
    )
    ...
    op.create_table(
        ...
        # SET NULL, not CASCADE: deleting a user must not rewrite a conversation by
        # removing their side of it.
        sa.Column(
            "sender_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("users.id", ondelete="SET NULL"),
    ...
    # Serves both "newest in this thread" and the "after X" polling cursor.
    op.create_index(
...
def patch_main(text: str) -> str:
    ...
    text = text.replace(
        import_anchor,
        import_anchor + "from app.api.v1 import messaging as messaging_routes\n",
        1,
    )
    ...
    new_line = re.sub(r"\b\w+_routes\.router", "messaging_routes.router", template, count=1)
...
def main() -> None:
    ...
    main_done = "messaging_routes" in main_text
    models_done = "app.models.messaging" in models_text
    ...
    if existing:
        ...
        target = VERSIONS / f"20260811_1400_{MIGRATION_ID}_{MIGRATION_NAME}.py"
