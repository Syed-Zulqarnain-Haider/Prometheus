        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          {items.map((n: NotificationItem) => (
            <button
              key={n.id}
              type="button"
              className={`block w-full border-b border-border-faint px-3 py-2 text-left hover:bg-accent ${
                n.read ? "opacity-60" : ""
              }`}
            ...
            >
              <div className="flex items-start gap-2">
                {!n.read && (
                  <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-[color:var(--color-accent)]" />
                )}
