...
async def active_announcements(context: CurrentUser, db: DbSession) -> list[AnnouncementOut]:
    ...
    mine = [
        row
        for row in rows
        if not row.audience_roles or any(role in row.audience_roles for role in context.roles)
    ]
    return [AnnouncementOut.model_validate(row, from_attributes=True) for row in mine]
